...
interface Dsa5Talent {
  name: string;
  value: number;
  eigenschaften: string[];
}

interface Dsa5Kampftechnik {
  name: string;
  at: number;
  pa: number;
}
...
interface Dsa5CharacterData {
  ...
  status: {
    wounds: Dsa5StatusValue;
    astralenergy: Dsa5StatusValue;
    karmaenergy: Dsa5StatusValue;
...
class PersistentCreatureIndex {
   ...
   */
  /**
   * Build enhanced creature index (system-aware: D&D 5e, PF2e, or DSA5)
   */
  private async buildEnhancedIndex(force = false): Promise<EnhancedCreatureIndex[]> {
    // Detect game system
    const gameSystem = (game as any).system.id;
    console.log(`[${this.moduleId}] Building index for system: ${gameSystem}`);

    // Route to appropriate system-specific builder
    if (gameSystem === 'dnd5e') {
      ...
      const errorMessage = `Unsupported game system: ${gameSystem}. Enhanced creature index only supports D&D 5e, Pathfinder 2e, and DSA 5.`;
      console.warn(`[${this.moduleId}] ${errorMessage}`);
  ...
  /**
   * Build DSA 5 enhanced creature index
   */
private async buildDsa5Index(_force = false): Promise<Dsa5CreatureIndex[]> {
  // Guard clause: Prevent concurrent builds
  if (this.buildInProgress) {
    console.warn(`[${this.moduleId}] DSA5 index build already in progress, skipping duplicate request`);
    return [];
  }
  ...
  this.buildInProgress = true
    ...
    try {
      ...
      for (const pack of actorPacks) {
        ...
        const result = await this.extractDsa5DataFromPack(pack);
        enhancedCreatures.push(...result.creatures);
        totalErrors += result.errors;
      }
      ...
      ui.notifications?.info(`Speichere DSA5 Index in Weltdatenbank... (${enhancedCreatures.length} Kreaturen)`);
  ...
  /**
 * Extract DSA 5 creature data from a single document
 */
private extractDsa5CreatureData(doc: any, pack: any): { creature: Dsa5CreatureIndex, errors: number } | null {
  try {
    ...
    // Culture ist jetzt Pflichtfeld mit Default "Keine"
    ...
    const sizeMap: Record<string, string> = {
      'winzig': 'tiny', 'klein': 'small', 'mittel': 'medium',
      'groß': 'large', 'riesig': 'huge', 'gigantisch': 'gargantuan'
    };
    size = sizeMap[size.toLowerCase()] || 'medium';
    ...
    const hasSpells = !!(system.status?.astralenergy?.max || system.status?.karmaenergy?.max ||
                        system.spells || system.liturgies || system.details?.tradition);
    ...
    const description = system.details?.biography?.value ?? system.details?.description?.value ??
                       system.biography?.value ?? system.description?.value ?? '';
    ...
    return {
      creature: {
        ...
        level: level,
        species: species,
        culture: culture,  // Jetzt immer vorhanden
    ...
    return {
      creature: {
        ...
        level: 0,
        species: 'Unbekannt',
        culture: 'Keine',  // Default auch im Fallback
        experience: 0,
        size: 'medium',
        lifePoints: 1,
        meleeDefense: 10,
...
export class FoundryDataAccess {
  ...
  /**
   * Type guard to check if creature is D&D 5e
   */
  private isDnD5eCreatureIndex(creature: EnhancedCreatureIndex): creature is DnD5eCreatureIndex {
    return 'challengeRating' in creature && !('level' in creature);
  }

  /**
   * Type guard to check if creature is Pathfinder 2e
   */
  private isPF2eCreatureIndex(creature: EnhancedCreatureIndex): creature is PF2eCreatureIndex {
    return 'level' in creature && 'rarity' in creature && !('species' in creature);
  }

  /**
   * Type guard to check if creature is DSA 5
   */
  private isDsa5CreatureIndex(creature: EnhancedCreatureIndex): creature is Dsa5CreatureIndex {
    return 'level' in creature && 'species' in creature;
  }
...
const results = filteredCreatures.map(creature => {
  // Save common properties first
  const baseCreature = creature as EnhancedCreatureIndex;
  
  // Type guards for result formatting
  const isDnd5e = this.isDnD5eCreatureIndex(creature);
  const isPF2e = this.isPF2eCreatureIndex(creature);
  const isDsa5 = this.isDsa5CreatureIndex(creature);

  // Build summary based on system
  let summary: string = `Creature from ${baseCreature.packLabel}`; // Default
  ...
  return {
    ...
    pack: baseCreature.pack,
    packLabel: baseCreature.packLabel,
  ...
  private passesEnhancedCriteria(creature: EnhancedCreatureIndex, criteria: any): boolean {
    // Use type guards to determine system
    if (this.isDnD5eCreatureIndex(creature)) {
      return this.passesDnD5eCriteria(creature, criteria);
    } else if (this.isPF2eCreatureIndex(creature)) {
      return this.passesPF2eCriteria(creature, criteria);
    } else if (this.isDsa5CreatureIndex(creature)) {
      return this.passesDsa5Criteria(creature, criteria);
    }
    return false;
  }

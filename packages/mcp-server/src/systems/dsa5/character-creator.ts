        ...
        try {
          ...
          if (filterBySpecies || filterByProfession) {
            ...
            for (const entry of actorEntries) {
              try {
                const fullData = await this.foundryClient.query(
                  'foundry-mcp-bridge.getCompendiumDocumentFull',
                  {
                    packId: pack.id,
                ...
                this.logger.warn(`Failed to load full data for entry ${entry.id}`, {
                  error: entryError,
                });
            ...
            this.logger.info(
              `Filtered entries: ${filteredEntries.length} out of ${actorEntries.length}`
            );
          }

...
export interface DSA5CreatureIndex extends SystemCreatureIndex {
  ...
  systemData: {
    ...
    level?: number;              // Experience level (1-7: Unerfahren bis Legendär)
    experiencePoints?: number;   // Total AP (Abenteuerpunkte)

    // Identity
    species?: string;            // Spezies (e.g., "Goblin", "Mensch", "Elf")
    culture?: string;            // Kultur (e.g., "Fjarninger", "Mittelländisch")
    profession?: string;         // Profession/Career

    // Physical attributes
    size?: string;               // Size category ("tiny", "small", "medium", etc.)
    lifePoints?: number;         // LeP (Lebensenergie)

    // Combat values
    meleeDefense?: number;       // Nahkampf-Verteidigung
    rangedDefense?: number;      // Fernkampf-Verteidigung
    armor?: number;              // Rüstungsschutz

    // Capabilities
    hasSpells: boolean;          // Has Zauber or Liturgien
    hasAstralEnergy?: boolean;   // Has AsP (Astralenergie)
    hasKarmaEnergy?: boolean;    // Has KaP (Karmaenergie)

    // Additional metadata
    traits?: string[];           // Special traits or abilities
